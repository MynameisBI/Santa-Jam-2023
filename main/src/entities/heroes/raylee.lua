...
function Raylee:initialize(slot)
    ...
    HeroEntity.initialize(self, slot, Images.heroes.raylee, 'Raylee', {'bigEar', 'droneMaestro'},
        ...
        Hero.Skill('Raylee',
            0, 0,
            function(hero, x, y)
                Hump.Gamestate.current():addEntity(AreaSkillEntity(Images.effects.rayleeSkill, hero,
                        {x = x, y = y}, 200, 60,
                        {damageType = 'reality', realityPowerRatio = 4, effects = {EnemyEffect('reduceRealityArmor', 3)}}, 0.3)) 
            end, true, 200, 60
        ))
